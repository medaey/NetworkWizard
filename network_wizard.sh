...
generateConfigFile() {
    local modelSwitch=$1
    cat > "$timestamp\_$modelSwitch.txt" << EOF
########################################################################
# FICHIER : $timestamp\_$modelSwitch.txt
# MODEL : $modelSwitch
#-----------------------------------------------------------------------
# VERSION : $timestamp
########################################################################
...
EOF
    globalSwitchConfig >> "$timestamp\_$modelSwitch.txt"
    vlanConfig >> "$timestamp\_$modelSwitch.txt"
}
